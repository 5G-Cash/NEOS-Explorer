            ...
            lib.syncLoop(vout.length, function (subloop) {
              ...
              lib.calculate_total(vout, function(total){
                ...
                if (typeof blocktype === 'string') {
                  var flag = blocktype.toLowerCase();
                  if (flag.indexOf('proof-of-stake') > -1 || flag === 'pos') {
                    type = 'PoS';
                  } else if (flag.indexOf('proof-of-work') > -1 || flag === 'pow') {
                    type = 'PoW';
                  }
                }
                if (type === '') {
  ...
  create_txs: function(block, cb) {
    is_locked("db_index", function (exists) {
      if (exists) {
        ...
        var blockType = block.flags || block.type;
        var process_txs = function(bt) {
          lib.syncLoop(block.tx.length, function (loop) {
            var i = loop.iteration();
            save_tx(block.tx[i], block.height, bt, function(err){
              if (err) {
                loop.next();
              } else {
        ...
        if (!blockType && block.tx.length > 0) {
          lib.get_rawtransaction(block.tx[0], function(raw){
            var bt = 'PoW';
            if (raw && raw.vin && raw.vin[0] && !raw.vin[0].coinbase) {
              bt = 'PoS';
            }
            process_txs(bt);
          });
        } else {
          process_txs(blockType);
        }
      }
    });
  },
                      ...
                      async.eachLimit(block.tx, task_limit_txs, function(txid, next_tx) {
                        Tx.findOne({txid: txid}, function(err, tx) {
                          if(tx) {
                            ...
                            save_tx(txid, block_height, bt, function(err){
                              if (err) {
                                console.log(err);
                              } else {
